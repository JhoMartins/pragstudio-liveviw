defmodule  LiveViewStudioWeb.LicenseLive do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      assign(socket,
        ...
        remaining_time: time_remaining(expiration_time)
      )
        ...
        <div class="content">
          <p class="m-4 font-semibold text-indigo-800">
            <%= if @remaining_time > 0 do %>
              <%= format_time(@remaining_time) %> left to save 20%
            <% else %>
              Expired!
            <% end %>
          </p>
  ...
  def handle_info(:tick, socket) do
    ...
    socket = assign(socket, :remaining_time, time_remaining(expiration_time))
    {:noreply, socket}
  end
